...
bool alm_flag = false;
bool tcm_flag = false;
bool vm21_reinv_rate_flag = false;
bool avr_flag = false;
bool imr_flag = false;
...
ofstream det_scenario_output; //20250407
...
//@@ START - generate_deterministic_scen
// Generate deterministic scenario                                                                                             
// Column:GENERATE_DETERMINISTIC_SCEN
//========================================================
double RATES_ECONOMY_UDF::rates_economy_generate_deterministic_scen(int t) {
//^^^
...
#line 1 "generate_deterministic_scen.RATES_ECONOMY.for"
if (t != 0)
	return NO_AVG;

xstring det_scenario_file = "";
xstring det_scenario_file_path = "<*scen_gen*>";

if (save_time_step_scen_files_defn == YES)
	det_scenario_file = replaceWildcards(det_scenario_file_path
	  + "det_scenario_file_" + xstring(proj_set_loop_num)
...
if (first_time)
{
	...
	struct stat buffer;
	int stat_val;
	stat_val = stat(det_scenario_file.c_str(), &buffer);
...
if (!det_scenario_output.is_open())
{
	det_scenario_output.setf(ios::fixed);
	det_scenario_output.precision(12);

	det_scenario_output.open(det_scenario_file);

	det_scenario_output	<<"Header,Description,Test File for Deterministic Scenario File Structure" << endl;
	det_scenario_output	<<"Header,Interpolation,CubicInterp" << endl;
	det_scenario_output <<"Header,MonthsPerPeriod,1" << endl;
	det_scenario_output <<"Header,Currency,USD" << endl;
	det_scenario_output <<"Header,YieldCurve,Swap,Terms:6-12-24-36-60-84-120-180-240-360,Annual,3,12" << endl;
	det_scenario_output <<"Header,Index,SP,Income Return-Index Growth,Annual,13,14" << endl;
	det_scenario_output <<"Header,MiscRate,MRMGAYield,Annual,15,15" << endl;
	det_scenario_output <<"Header,MiscRate,RiskFree,Annual,16,16" << endl;
	det_scenario_output <<"Header,End " << endl;
	det_scenario_output 
	<< "Scen" << ","
	<< "Month" << ","
	<< "Swap_6" << ","
	...
	<< "SP_Growth" << ","
	<< "MRMGAYield_MiscRate" << ","
	<< "RiskFree_MiscRate" << endl;

}

double current_fwd_rate_1 = 0.0;
double current_fwd_rate_6 = 0.0;
double current_fwd_rate_12 = 0.0;
double current_fwd_rate_24 = 0.0;
double current_fwd_rate_36 = 0.0;
double current_fwd_rate_60 = 0.0;
double current_fwd_rate_84 = 0.0;
double current_fwd_rate_120 = 0.0;
double current_fwd_rate_180 = 0.0;
double current_fwd_rate_240 = 0.0;
double current_fwd_rate_360 = 0.0;

for (int i = 0; i <= 720; i++)
{
	double term = i/12.;
	double test_rate = get_int_rate(0, "Swap", GET_YIELD_RATE, term, 0, EFFECTIVE_ANNUAL, NO_SHIFT);		
}
...
for (int i = 0; i <= 360; i++)
{
	...
	if (i == 0)
	{	
		current_fwd_rate_1 = get_int_rate(0, "Swap", GET_YIELD_RATE, 1.0/12.0, 0, EFFECTIVE_ANNUAL, NO_SHIFT);									  
		current_fwd_rate_6 = get_int_rate(0, "Swap", GET_YIELD_RATE, 6.0/12.0, 0, EFFECTIVE_ANNUAL, NO_SHIFT);		
		current_fwd_rate_12 = get_int_rate(0, "Swap", GET_YIELD_RATE, 1.0, 0, EFFECTIVE_ANNUAL, NO_SHIFT);
		current_fwd_rate_24 = get_int_rate(0, "Swap", GET_YIELD_RATE, 2.0, 0, EFFECTIVE_ANNUAL, NO_SHIFT);
		current_fwd_rate_36 = get_int_rate(0, "Swap", GET_YIELD_RATE, 3.0, 0, EFFECTIVE_ANNUAL, NO_SHIFT);
		current_fwd_rate_60 = get_int_rate(0, "Swap", GET_YIELD_RATE, 5.0, 0, EFFECTIVE_ANNUAL, NO_SHIFT);
		current_fwd_rate_84 = get_int_rate(0, "Swap", GET_YIELD_RATE, 7.0, 0, EFFECTIVE_ANNUAL, NO_SHIFT);
		current_fwd_rate_120 = get_int_rate(0, "Swap", GET_YIELD_RATE, 10.0, 0, EFFECTIVE_ANNUAL, NO_SHIFT);
		current_fwd_rate_180 = get_int_rate(0, "Swap", GET_YIELD_RATE, 15.0, 0, EFFECTIVE_ANNUAL, NO_SHIFT);
		current_fwd_rate_240 = get_int_rate(0, "Swap", GET_YIELD_RATE, 20.0, 0, EFFECTIVE_ANNUAL, NO_SHIFT);							
		current_fwd_rate_360 = get_int_rate(0, "Swap", GET_YIELD_RATE, 30.0, 0, EFFECTIVE_ANNUAL, NO_SHIFT);

	}
	else 
	{
		current_fwd_rate_1 = calc_fwrd_rate_crbg(i * (1.0 / 12.0), 1.0 / 12.0);
		current_fwd_rate_6 = calc_fwrd_rate_crbg(i * (1.0 / 12.0), 6.0 / 12.0);
		current_fwd_rate_12 = calc_fwrd_rate_crbg(i * (1.0 / 12.0), 1.0);
		current_fwd_rate_24 = calc_fwrd_rate_crbg(i * (1.0 / 12.0), 2.0);
		current_fwd_rate_36 = calc_fwrd_rate_crbg(i * (1.0 / 12.0), 3.0);
		current_fwd_rate_60 = calc_fwrd_rate_crbg(i * (1.0 / 12.0), 5.0);
		current_fwd_rate_84 = calc_fwrd_rate_crbg(i * (1.0 / 12.0), 7.0);
		current_fwd_rate_120 = calc_fwrd_rate_crbg(i * (1.0 / 12.0), 10.0);
		current_fwd_rate_180 = calc_fwrd_rate_crbg(i * (1.0 / 12.0), 15.0);
		current_fwd_rate_240 = calc_fwrd_rate_crbg(i * (1.0 / 12.0), 20.0);
		current_fwd_rate_360 = calc_fwrd_rate_crbg(i * (1.0 / 12.0), 30.0);
	...
	<< 1 << "," //Scen
	<< i << "," //Month
	<< current_fwd_rate_6 << "," //Swap_6
	<< current_fwd_rate_12 << "," //Swap_12
	<< current_fwd_rate_24 << "," //Swap_24
	<< current_fwd_rate_36 << "," //Swap_36
	<< current_fwd_rate_60 << "," //Swap_60
	<< current_fwd_rate_84 << "," //Swap_84
	<< current_fwd_rate_120 << "," //Swap_120
	<< current_fwd_rate_180 << "," //Swap_180
	<< current_fwd_rate_240 << "," //Swap_240
	<< current_fwd_rate_360 << "," //Swap_360
	<< 0.0 << "," //SP_Income
	<< current_fwd_rate_1 << "," //SP_Growth
	<< current_fwd_rate_1 << "," //MRMGAYield_MiscRate
	<< current_fwd_rate_1 << endl; //RiskFree_MiscRate
...
det_scenario_output.close(); //20250407
...
double RATES_ECONOMY::calc_fwrd_rate_crbg(double rate_term_input, double fwd_term)
{
	double rate_start_term = get_int_rate(0, "Swap", GET_YIELD_RATE, rate_term_input, 0, NOMINAL_SEMIANNUAL, NO_SHIFT);
	
	double rate_end_term = get_int_rate(0, "Swap", GET_YIELD_RATE, rate_term_input + fwd_term, 0, NOMINAL_SEMIANNUAL, NO_SHIFT);
	
	double rate = (pow(pow(1.0 + rate_end_term
						  / 2.,
	...
	return convert_rate_basis(rate, NOMINAL_SEMIANNUAL, EFFECTIVE_ANNUAL);
}
...
if (rate_type == GET_SPOT_SPREAD)
	...
	if (quality_id_input == "Govt")
		...
		if (find(yld_curve_list.begin(), yld_curve_list.end(), quality_id_input) != yld_curve_list.end())
			...
			if(!valn_flag) //20240814 Use proper flag to apply deterministic spread for PBA run.
			{
				quality_id = quality_id_input;//WTW - Gen2 - Mutating Lookup Term
				rate_term = rate_term_input;//WTW - Gen2 - Mutating Lookup Term
			...
			else
			{
				quality_id = quality_id_input;
				rate_term = rate_term_input;
				date_lookup_gen2 = date(t);
				sprd = credit_sprd_deterministic;
			}
...
FunctionPtr RATES_ECONOMY::RegisterTerminatorOnce = []() {
...
const CashFlowCommonData RATES_ECONOMY::mCFStaticData_0[] = 
{
	...
	CashFlowCommonData(10, "generate_deterministic_scen", "rates_economy_generate_deterministic_scen",  "generate_deterministic_scen",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&RATES_ECONOMY_UDF::rates_economy_generate_deterministic_scen, 'E','N', '3', 'P', (size_t)&modelOffset->generate_deterministic_scen),
	CashFlowCommonData(11, "infl_rate_annual", "rates_economy_infl_rate_annual",  "infl_rate_annual",  CashFlowCommonData::AVG,
                     (dPFi)(dPXi)&RATES_ECONOMY_UDF::rates_economy_infl_rate_annual, 'E','N', '3', 'P', (size_t)&modelOffset->infl_rate_annual),
	CashFlowCommonData(12, "initialize", "rates_economy_initialize",  "initialize",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&RATES_ECONOMY_UDF::rates_economy_initialize, 'E','N', '3', 'N', (size_t)&modelOffset->initialize),
	CashFlowCommonData(13, "startup", "rates_economy_startup",  "startup",  CashFlowCommonData::AVG,
                     (dPFi)(dPXi)&RATES_ECONOMY_UDF::virtual_startup, 'E','N', '3', 'N', (size_t)&modelOffset->startup),
	CashFlowCommonData(14, "ten_year_treasury_annualized_return_cumul_max_aig", "rates_economy_ten_year_treasury_annualized_return_cumul_max_aig",  "ten_year_treasury_annualized_return_cumul_max_aig",  CashFlowCommonData::SUM,
	...
	CashFlowCommonData(15, "ten_year_treasury_annualized_return_max_aig", "rates_economy_ten_year_treasury_annualized_return_max_aig",  "ten_year_treasury_annualized_return_max_aig",  CashFlowCommonData::SUM,
	...
	CashFlowCommonData(16, "ten_year_treasury_annualized_return_three_mth_avg_aig", "rates_economy_ten_year_treasury_annualized_return_three_mth_avg_aig",  "ten_year_treasury_annualized_return_three_mth_avg_aig",  CashFlowCommonData::SUM,
	...
	CashFlowCommonData(17, "yield_rate_10_year", "rates_economy_yield_rate_10_year",  "yield_rate_10_year",  CashFlowCommonData::AVG,
                     (dPFi)(dPXi)&RATES_ECONOMY_UDF::rates_economy_yield_rate_10_year, 'E','N', '3', 'P', (size_t)&modelOffset->yield_rate_10_year),
	CashFlowCommonData(18, "yield_rate_90_day", "rates_economy_yield_rate_90_day",  "yield_rate_90_day",  CashFlowCommonData::AVG,
...
const CashFlowCommonData* RATES_ECONOMY::mCFStaticData[] = {
	...
	&RATES_ECONOMY::mCFStaticData_0[18],
	nullptr};
...
RATES_ECONOMY::RATES_ECONOMY(const xstring &modelClassName,
		...
		const char *name, ModelClass* arrayPersistentObj) : ModelClass(18, RATES_ECONOMY::descriptorTable, arrayPersistentObj), Variable(*this)
  , sm_bond_is(0)
  , sm_bond_pv(0)
  , sm_bond_ym(0)
  , sm_mtg_is(0)
...
#pragma warning(pop)
   ...
   mMOCD = descriptorTable;
	...
	for (int cf_no = 1; cf_no <= 18; cf_no++)
#ifdef CF_MEMORY
		setPtr_col(cf_no, 0);
#else
