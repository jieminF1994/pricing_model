...
//20240321 US library 6.0 bug fix
return assets_accum_inv(t) 
		+ hedge_mkt_val(t);
...
//@@ START - assets_accum_inv
// Accumulated Assets                                                                                             
// Column:ASSETS_ACCUM_INV
//========================================================
double ANN_PBA_PBA_UDF::ann_pba_pba_assets_accum_inv(int t) {
//^^^
...
#line 1 "assets_accum_inv.ANN_PBA_PBA.for"
//20240321 New column. US library 6.0 bug fix
if (t < start_period || t > final_period)
	return NO_AVG;
...
//^^^
if ( isRebaseClone && sm_peer_model  && t <= rebase_period ){
	return rebase_value(9,"cash_flow_int",t);
}
...
//^^^
if ( isRebaseClone && sm_peer_model  && t <= rebase_period ){
	return rebase_value(10,"cash_flow_rate_mthly",t);
}
...
//^^^
if ( isRebaseClone && sm_peer_model  && t <= rebase_period ){
	return rebase_value(15,"direct_cash_flow_int",t);
}
...
//^^^
if ( isRebaseClone && sm_peer_model  && t <= rebase_period ){
	return rebase_value(16,"direct_imr_incr",t);
}
...
if (disc_rate_method == "NAER")
{
	return pow(1.0 + company->asset_yld_vm21_reinv_rate(t), 1.0 / 12.0);
}
else
{
	throw FatalError("This model is not currently set up to handle any discount rate method other than NAER");
}
/*
This model is not currently set up to handle any discount rate method other than NAER
if (disc_rate_method == "AG33")
	return pow(1.0 + carvm_int_rate, 1.0 / 12.0);
*/
...
if((res_cap_calc_defn_input == RESERVES) && (t < start_period || t > final_period))
	return NO_AVG;

if((res_cap_calc_defn_input == RESERVES) && t >= start_period && t < final_period)
{
	double opt_value = seg->hedge_mkt_val(t);
	return opt_value;
}
...
//^^^
if ( isRebaseClone && sm_peer_model  && t <= rebase_period ){
	return rebase_value(30,"imr_incr",t);
}
...
//20240321 US library 6.0 bug fix
return assets_accum_inv(t - 1) * asset_yld_mthly(t);
...
//^^^
if ( isRebaseClone && sm_peer_model  && t <= rebase_period ){
	return rebase_value(33,"pimr_adj_factor",t);
}
...
//^^^
if ( isRebaseClone && sm_peer_model  && t <= rebase_period ){
	return rebase_value(38,"reins_cash_flow_int",t);
}
...
//^^^
if ( isRebaseClone && sm_peer_model  && t <= rebase_period ){
	return rebase_value(40,"reins_imr_incr",t);
}
...
//^^^
if ( isRebaseClone && sm_peer_model  && t <= rebase_period ){
	return rebase_value(45,"reins_tax_res_incr",t);
}
...
#if defined(__FA_LIAB_H_)
if (this == fa_ann_pba && pricing_flag)
	group_name = fa->mp_id
	             + "|"
	             + res_cap_calc_defn 
	             + "|" 
...
#endif
#if defined(__FIA_LIAB_H_)
if (this == fia_ann_pba && pricing_flag)
...
#endif
#if defined(__PA_LIAB_H_)
if (this == pa_ann_pba && pricing_flag)
	group_name = pa->mp_id
	             + "|"
	             + res_cap_calc_defn 
	             + "|" 
...
//^^^
if ( isRebaseClone && sm_peer_model  && t <= rebase_period ){
	return rebase_value(53,"tax_capzd_prem",t);
}
...
//^^^
if ( isRebaseClone && sm_peer_model  && t <= rebase_period ){
	return rebase_value(54,"tax_capzd_prem_amortzn",t);
}
...
//^^^
if ( isRebaseClone && sm_peer_model  && t <= rebase_period ){
	return rebase_value(58,"tax_res_incr",t);
}
...
const CashFlowCommonData ANN_PBA_PBA::mCFStaticData_0[] = 
{
	...
	CashFlowCommonData(3, "assets_accum_inv", "ann_pba_pba_assets_accum_inv",  "assets_accum_inv",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_assets_accum_inv, 'E','N', '3', 'P', (size_t)&modelOffset->assets_accum_inv),
	CashFlowCommonData(4, "cal_mth", "ann_pba_pba_cal_mth",  "cal_mth",  CashFlowCommonData::AVG,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_cal_mth, 'E','N', '3', 'C', (size_t)&modelOffset->cal_mth),
	CashFlowCommonData(5, "cal_yr", "ann_pba_pba_cal_yr",  "cal_yr",  CashFlowCommonData::AVG,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_cal_yr, 'E','N', '3', 'C', (size_t)&modelOffset->cal_yr),
	CashFlowCommonData(6, "cal_yr_relative", "ann_pba_pba_cal_yr_relative",  "cal_yr_relative",  CashFlowCommonData::AVG,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_cal_yr_relative, 'E','N', '3', 'C', (size_t)&modelOffset->cal_yr_relative),
	CashFlowCommonData(7, "cap_gain", "ann_pba_pba_cap_gain",  "cap_gain",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_cap_gain, 'E','Y', '3', 'P', (size_t)&modelOffset->cap_gain),
	CashFlowCommonData(8, "cash_flow", "ann_pba_pba_cash_flow",  "cash_flow",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_cash_flow, 'E','Y', '3', 'P', (size_t)&modelOffset->cash_flow),
	CashFlowCommonData(9, "cash_flow_int", "ann_pba_pba_cash_flow_int",  "cash_flow_int",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_cash_flow_int, 'E','Y', '3', 'C', (size_t)&modelOffset->cash_flow_int),
	CashFlowCommonData(10, "cash_flow_rate_mthly", "ann_pba_pba_cash_flow_rate_mthly",  "cash_flow_rate_mthly",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_cash_flow_rate_mthly, 'E','N', '3', 'C', (size_t)&modelOffset->cash_flow_rate_mthly),
	CashFlowCommonData(11, "date", "ann_pba_pba_date",  "date",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_date, 'E','N', '3', 'P', (size_t)&modelOffset->date),
	CashFlowCommonData(12, "def_accum", "ann_pba_pba_def_accum",  "def_accum",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_def_accum, 'E','N', '3', 'P', (size_t)&modelOffset->def_accum),
	CashFlowCommonData(13, "direct_cap_gain", "ann_pba_pba_direct_cap_gain",  "direct_cap_gain",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_direct_cap_gain, 'E','Y', '3', 'P', (size_t)&modelOffset->direct_cap_gain),
	CashFlowCommonData(14, "direct_cash_flow", "ann_pba_pba_direct_cash_flow",  "direct_cash_flow",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_direct_cash_flow, 'E','Y', '3', 'P', (size_t)&modelOffset->direct_cash_flow),
	CashFlowCommonData(15, "direct_cash_flow_int", "ann_pba_pba_direct_cash_flow_int",  "direct_cash_flow_int",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_direct_cash_flow_int, 'E','Y', '3', 'C', (size_t)&modelOffset->direct_cash_flow_int),
	CashFlowCommonData(16, "direct_imr_incr", "ann_pba_pba_direct_imr_incr",  "direct_imr_incr",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_direct_imr_incr, 'E','Y', '3', 'C', (size_t)&modelOffset->direct_imr_incr),
	CashFlowCommonData(17, "direct_inv_inc", "ann_pba_pba_direct_inv_inc",  "direct_inv_inc",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_direct_inv_inc, 'E','Y', '3', 'P', (size_t)&modelOffset->direct_inv_inc),
	CashFlowCommonData(18, "direct_prof", "ann_pba_pba_direct_prof",  "direct_prof",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_direct_prof, 'E','Y', '3', 'P', (size_t)&modelOffset->direct_prof),
	CashFlowCommonData(19, "direct_res_incr", "ann_pba_pba_direct_res_incr",  "direct_res_incr",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_direct_res_incr, 'E','Y', '3', 'P', (size_t)&modelOffset->direct_res_incr),
	CashFlowCommonData(20, "direct_tax", "ann_pba_pba_direct_tax",  "direct_tax",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_direct_tax, 'E','Y', '3', 'P', (size_t)&modelOffset->direct_tax),
	CashFlowCommonData(21, "disc_rate_accum", "ann_pba_pba_disc_rate_accum",  "disc_rate_accum",  CashFlowCommonData::AVG,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_disc_rate_accum, 'E','N', '3', 'P', (size_t)&modelOffset->disc_rate_accum),
	CashFlowCommonData(22, "disc_rate_mthly", "ann_pba_pba_disc_rate_mthly",  "disc_rate_mthly",  CashFlowCommonData::AVG,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_disc_rate_mthly, 'E','N', '3', 'P', (size_t)&modelOffset->disc_rate_mthly),
	CashFlowCommonData(23, "eff_cap_gains_rate", "ann_pba_pba_eff_cap_gains_rate",  "eff_cap_gains_rate",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_eff_cap_gains_rate, 'E','N', '3', 'P', (size_t)&modelOffset->eff_cap_gains_rate),
	CashFlowCommonData(24, "hedge_cash_flow", "ann_pba_pba_hedge_cash_flow",  "hedge_cash_flow",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_hedge_cash_flow, 'E','Y', '3', 'P', (size_t)&modelOffset->hedge_cash_flow),
	CashFlowCommonData(25, "hedge_cost", "ann_pba_pba_hedge_cost",  "hedge_cost",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_hedge_cost, 'E','Y', '3', 'P', (size_t)&modelOffset->hedge_cost),
	CashFlowCommonData(26, "hedge_mkt_val", "ann_pba_pba_hedge_mkt_val",  "hedge_mkt_val",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_hedge_mkt_val, 'E','N', '3', 'P', (size_t)&modelOffset->hedge_mkt_val),
	CashFlowCommonData(27, "hedge_mkt_val_incr", "ann_pba_pba_hedge_mkt_val_incr",  "hedge_mkt_val_incr",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_hedge_mkt_val_incr, 'E','Y', '3', 'P', (size_t)&modelOffset->hedge_mkt_val_incr),
	CashFlowCommonData(28, "hedging_e_factor", "ann_pba_pba_hedging_e_factor",  "hedging_e_factor",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_hedging_e_factor, 'E','Y', '3', 'P', (size_t)&modelOffset->hedging_e_factor),
	CashFlowCommonData(29, "imr", "ann_pba_pba_imr",  "imr",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_imr, 'E','N', '3', 'C', (size_t)&modelOffset->imr),
	CashFlowCommonData(30, "imr_incr", "ann_pba_pba_imr_incr",  "imr_incr",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_imr_incr, 'E','Y', '3', 'C', (size_t)&modelOffset->imr_incr),
	CashFlowCommonData(31, "initialize", "ann_pba_pba_initialize",  "initialize",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_initialize, 'E','N', '3', 'N', (size_t)&modelOffset->initialize),
	CashFlowCommonData(32, "inv_inc", "ann_pba_pba_inv_inc",  "inv_inc",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_inv_inc, 'E','Y', '3', 'P', (size_t)&modelOffset->inv_inc),
	CashFlowCommonData(33, "pimr_adj_factor", "ann_pba_pba_pimr_adj_factor",  "pimr_adj_factor",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_pimr_adj_factor, 'E','N', '3', 'C', (size_t)&modelOffset->pimr_adj_factor),
	CashFlowCommonData(34, "realzd_cap_gain", "ann_pba_pba_realzd_cap_gain",  "realzd_cap_gain",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_realzd_cap_gain, 'E','Y', '3', 'P', (size_t)&modelOffset->realzd_cap_gain),
	CashFlowCommonData(35, "realzd_cap_gain_bef_sale", "ann_pba_pba_realzd_cap_gain_bef_sale",  "realzd_cap_gain_bef_sale",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_realzd_cap_gain_bef_sale, 'E','Y', '3', 'P', (size_t)&modelOffset->realzd_cap_gain_bef_sale),
	CashFlowCommonData(36, "reins_cap_gain", "ann_pba_pba_reins_cap_gain",  "reins_cap_gain",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_cap_gain, 'E','Y', '3', 'P', (size_t)&modelOffset->reins_cap_gain),
	CashFlowCommonData(37, "reins_cash_flow", "ann_pba_pba_reins_cash_flow",  "reins_cash_flow",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_cash_flow, 'E','Y', '3', 'P', (size_t)&modelOffset->reins_cash_flow),
	CashFlowCommonData(38, "reins_cash_flow_int", "ann_pba_pba_reins_cash_flow_int",  "reins_cash_flow_int",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_cash_flow_int, 'E','Y', '3', 'C', (size_t)&modelOffset->reins_cash_flow_int),
	CashFlowCommonData(39, "reins_cost", "ann_pba_pba_reins_cost",  "reins_cost",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_cost, 'E','Y', '3', 'P', (size_t)&modelOffset->reins_cost),
	CashFlowCommonData(40, "reins_imr_incr", "ann_pba_pba_reins_imr_incr",  "reins_imr_incr",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_imr_incr, 'E','Y', '3', 'C', (size_t)&modelOffset->reins_imr_incr),
	CashFlowCommonData(41, "reins_inv_inc", "ann_pba_pba_reins_inv_inc",  "reins_inv_inc",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_inv_inc, 'E','Y', '3', 'P', (size_t)&modelOffset->reins_inv_inc),
	CashFlowCommonData(42, "reins_res", "ann_pba_pba_reins_res",  "reins_res",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_res, 'E','N', '3', 'P', (size_t)&modelOffset->reins_res),
	CashFlowCommonData(43, "reins_res_incr", "ann_pba_pba_reins_res_incr",  "reins_res_incr",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_res_incr, 'E','Y', '3', 'P', (size_t)&modelOffset->reins_res_incr),
	CashFlowCommonData(44, "reins_tax_res", "ann_pba_pba_reins_tax_res",  "reins_tax_res",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_tax_res, 'E','N', '3', 'P', (size_t)&modelOffset->reins_tax_res),
	CashFlowCommonData(45, "reins_tax_res_incr", "ann_pba_pba_reins_tax_res_incr",  "reins_tax_res_incr",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_tax_res_incr, 'E','Y', '3', 'C', (size_t)&modelOffset->reins_tax_res_incr),
	CashFlowCommonData(46, "scen_amt", "ann_pba_pba_scen_amt",  "scen_amt",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_scen_amt, 'E','N', '3', 'P', (size_t)&modelOffset->scen_amt),
	CashFlowCommonData(47, "scen_greatest_pv", "ann_pba_pba_scen_greatest_pv",  "scen_greatest_pv",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_scen_greatest_pv, 'E','N', '3', 'P', (size_t)&modelOffset->scen_greatest_pv),
	CashFlowCommonData(48, "starting_assets", "ann_pba_pba_starting_assets",  "starting_assets",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_starting_assets, 'E','N', '3', 'P', (size_t)&modelOffset->starting_assets),
	CashFlowCommonData(49, "startup", "ann_pba_pba_startup",  "startup",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::virtual_startup, 'E','N', '3', 'N', (size_t)&modelOffset->startup),
	CashFlowCommonData(50, "stoch_amt", "ann_pba_pba_stoch_amt",  "stoch_amt",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_stoch_amt, 'E','N', '3', 'P', (size_t)&modelOffset->stoch_amt),
	CashFlowCommonData(51, "tax", "ann_pba_pba_tax",  "tax",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_tax, 'E','Y', '3', 'P', (size_t)&modelOffset->tax),
	CashFlowCommonData(52, "tax_adj_ratio", "ann_pba_pba_tax_adj_ratio",  "tax_adj_ratio",  CashFlowCommonData::AVG,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_tax_adj_ratio, 'E','N', '3', 'P', (size_t)&modelOffset->tax_adj_ratio),
	CashFlowCommonData(53, "tax_capzd_prem", "ann_pba_pba_tax_capzd_prem",  "tax_capzd_prem",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_tax_capzd_prem, 'E','Y', '3', 'C', (size_t)&modelOffset->tax_capzd_prem),
	CashFlowCommonData(54, "tax_capzd_prem_amortzn", "ann_pba_pba_tax_capzd_prem_amortzn",  "tax_capzd_prem_amortzn",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_tax_capzd_prem_amortzn, 'E','Y', '3', 'C', (size_t)&modelOffset->tax_capzd_prem_amortzn),
	CashFlowCommonData(55, "tax_cash_flow", "ann_pba_pba_tax_cash_flow",  "tax_cash_flow",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_tax_cash_flow, 'E','Y', '3', 'P', (size_t)&modelOffset->tax_cash_flow),
	CashFlowCommonData(56, "tax_cash_flow_bef_sale", "ann_pba_pba_tax_cash_flow_bef_sale",  "tax_cash_flow_bef_sale",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_tax_cash_flow_bef_sale, 'E','Y', '3', 'P', (size_t)&modelOffset->tax_cash_flow_bef_sale),
	CashFlowCommonData(57, "tax_res", "ann_pba_pba_tax_res",  "tax_res",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_tax_res, 'E','N', '3', 'P', (size_t)&modelOffset->tax_res),
	CashFlowCommonData(58, "tax_res_incr", "ann_pba_pba_tax_res_incr",  "tax_res_incr",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_tax_res_incr, 'E','Y', '3', 'C', (size_t)&modelOffset->tax_res_incr),
	CashFlowCommonData(59, "tax_ytd", "ann_pba_pba_tax_ytd",  "tax_ytd",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_tax_ytd, 'E','N', '3', 'P', (size_t)&modelOffset->tax_ytd),
	CashFlowCommonData(60, "tax_ytd_bef_sale", "ann_pba_pba_tax_ytd_bef_sale",  "tax_ytd_bef_sale",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_tax_ytd_bef_sale, 'E','N', '3', 'P', (size_t)&modelOffset->tax_ytd_bef_sale),
	CashFlowCommonData(61, "taxable_inc", "ann_pba_pba_taxable_inc",  "taxable_inc",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_taxable_inc, 'E','Y', '3', 'P', (size_t)&modelOffset->taxable_inc),
	CashFlowCommonData(62, "taxable_inc_bef_sale", "ann_pba_pba_taxable_inc_bef_sale",  "taxable_inc_bef_sale",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_taxable_inc_bef_sale, 'E','Y', '3', 'P', (size_t)&modelOffset->taxable_inc_bef_sale),
	CashFlowCommonData(63, "taxable_inc_ytd", "ann_pba_pba_taxable_inc_ytd",  "taxable_inc_ytd",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_taxable_inc_ytd, 'E','N', '3', 'P', (size_t)&modelOffset->taxable_inc_ytd),
	CashFlowCommonData(64, "taxable_inc_ytd_bef_sale", "ann_pba_pba_taxable_inc_ytd_bef_sale",  "taxable_inc_ytd_bef_sale",  CashFlowCommonData::SUM,
                     (dPFi)(dPXi)&ANN_PBA_PBA_UDF::ann_pba_pba_taxable_inc_ytd_bef_sale, 'E','N', '3', 'P', (size_t)&modelOffset->taxable_inc_ytd_bef_sale),
	CashFlowCommonData(65, "working_res", "ann_pba_pba_working_res",  "working_res",  CashFlowCommonData::SUM,
...
const CashFlowCommonData* ANN_PBA_PBA::mCFStaticData[] = {
	...
	&ANN_PBA_PBA::mCFStaticData_0[65],
	nullptr};
...
TableMgr<VariantTable> ANN_PBA_PBA::mgr_;
	...
	Descriptor(1, Attribute::STRING,	"disc_rate_method", -1, (size_t)&modelOffset->disc_rate_method,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(2, Attribute::STR_ENUM,	"hedging_cash_flow_defn", -1, (size_t)&modelOffset->hedging_cash_flow_defn,
				Descriptor::NOT_SHARED, false, false, (size_t)0, &hedging_cash_flow_defnEnumList, Feature(true)),
	Descriptor(3, Attribute::STR_ENUM,	"hedging_defn", -1, (size_t)&modelOffset->hedging_defn,
				Descriptor::NOT_SHARED, false, false, (size_t)0, &hedging_defnEnumList, Feature(true)),
	Descriptor(4, Attribute::STR_ENUM,	"hedging_defn_input", -1, (size_t)&modelOffset->hedging_defn_input,
				Descriptor::NOT_SHARED, false, false, (size_t)0, &hedging_defn_inputEnumList, Feature(true)),
	Descriptor(5, Attribute::DOUBLE,	"hedging_e_factor_stoch_cap", Descriptor::NOT_INDEXED, (size_t)&modelOffset->hedging_e_factor_stoch_cap,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(6, Attribute::DOUBLE,	"hedging_e_factor_stoch_res", Descriptor::NOT_INDEXED, (size_t)&modelOffset->hedging_e_factor_stoch_res,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(7, Attribute::STRING,	"hedging_output_file_best_estimate_stoch_cap", -1, (size_t)&modelOffset->hedging_output_file_best_estimate_stoch_cap,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(8, Attribute::STRING,	"hedging_output_file_best_estimate_stoch_res", -1, (size_t)&modelOffset->hedging_output_file_best_estimate_stoch_res,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(9, Attribute::STRING,	"hedging_output_file_existing_hedges_stoch_cap", -1, (size_t)&modelOffset->hedging_output_file_existing_hedges_stoch_cap,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(10, Attribute::STRING,	"hedging_output_file_existing_hedges_stoch_res", -1, (size_t)&modelOffset->hedging_output_file_existing_hedges_stoch_res,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(11, Attribute::DOUBLE,	"hedging_stat_scale_factor_max_stoch_cap", Descriptor::NOT_INDEXED, (size_t)&modelOffset->hedging_stat_scale_factor_max_stoch_cap,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(12, Attribute::DOUBLE,	"hedging_stat_scale_factor_max_stoch_res", Descriptor::NOT_INDEXED, (size_t)&modelOffset->hedging_stat_scale_factor_max_stoch_res,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(13, Attribute::DOUBLE,	"init_asset_pct_input", Descriptor::NOT_INDEXED, (size_t)&modelOffset->init_asset_pct_input,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(14, Attribute::STRING,	"init_assets_defn", -1, (size_t)&modelOffset->init_assets_defn,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(15, Attribute::DOUBLE,	"init_assets_input", Descriptor::NOT_INDEXED, (size_t)&modelOffset->init_assets_input,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(16, Attribute::STR_ENUM,	"pba_code", -1, (size_t)&modelOffset->pba_code,
				Descriptor::NOT_SHARED, false, false, (size_t)0, &pba_codeEnumList, Feature(true)),
	Descriptor(17, Attribute::STRING,	"pba_cte_code", -1, (size_t)&modelOffset->pba_cte_code,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(18, Attribute::STRING,	"proj_date", -1, (size_t)&modelOffset->proj_date,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(19, Attribute::DOUBLE,	"rbc_c3_cte98_factor", Descriptor::NOT_INDEXED, (size_t)&modelOffset->rbc_c3_cte98_factor,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(20, Attribute::DOUBLE,	"rbc_c3_cte98_factor_input", Descriptor::NOT_INDEXED, (size_t)&modelOffset->rbc_c3_cte98_factor_input,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(21, Attribute::STR_ENUM,	"reins_defn", -1, (size_t)&modelOffset->reins_defn,
				Descriptor::NOT_SHARED, false, false, (size_t)0, &reins_defnEnumList, Feature(true)),
	Descriptor(22, Attribute::STR_ENUM,	"reins_defn_input", -1, (size_t)&modelOffset->reins_defn_input,
				Descriptor::NOT_SHARED, false, false, (size_t)0, &reins_defn_inputEnumList, Feature(true)),
	Descriptor(23, Attribute::STR_ENUM,	"reins_gross_up_defn", -1, (size_t)&modelOffset->reins_gross_up_defn,
				Descriptor::NOT_SHARED, false, false, (size_t)0, &reins_gross_up_defnEnumList, Feature(true)),
	Descriptor(24, Attribute::STRING,	"res_assumption_set", -1, (size_t)&modelOffset->res_assumption_set,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(25, Attribute::STR_ENUM,	"res_cap_calc_defn", -1, (size_t)&modelOffset->res_cap_calc_defn,
				Descriptor::NOT_SHARED, false, false, (size_t)0, &res_cap_calc_defnEnumList, Feature(true)),
	Descriptor(26, Attribute::STR_ENUM,	"res_cap_calc_defn_input", -1, (size_t)&modelOffset->res_cap_calc_defn_input,
				Descriptor::NOT_SHARED, false, false, (size_t)0, &res_cap_calc_defn_inputEnumList, Feature(true)),
	Descriptor(27, Attribute::DOUBLE,	"rila_ind", Descriptor::NOT_INDEXED, (size_t)&modelOffset->rila_ind,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(28, Attribute::DOUBLE,	"rila_opt_inefficiency", Descriptor::NOT_INDEXED, (size_t)&modelOffset->rila_opt_inefficiency,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(29, Attribute::DOUBLE,	"rila_opt_inefficiency_input", Descriptor::NOT_INDEXED, (size_t)&modelOffset->rila_opt_inefficiency_input,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(30, Attribute::STRING,	"seg_id", -1, (size_t)&modelOffset->seg_id,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(31, Attribute::STR_ENUM,	"tax_defn_stoch_cap", -1, (size_t)&modelOffset->tax_defn_stoch_cap,
				Descriptor::NOT_SHARED, false, false, (size_t)0, &tax_defn_stoch_capEnumList, Feature(true)),
	Descriptor(32, Attribute::STRING,	"valn_results_stoch_cap", -1, (size_t)&modelOffset->valn_results_stoch_cap,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(33, Attribute::STRING,	"valn_results_stoch_res", -1, (size_t)&modelOffset->valn_results_stoch_res,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(34, Attribute::STR_ENUM,	"working_res_defn_input", -1, (size_t)&modelOffset->working_res_defn_input,
				Descriptor::NOT_SHARED, false, false, (size_t)0, &working_res_defn_inputEnumList, Feature(true)),
	Descriptor(35, Attribute::INT,	"msnumelement", -1, (size_t)&modelOffset->msnumelement,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature()),
	Descriptor(36, Attribute::SCALAR_DOUBLE,	"cte_pct", -1, (size_t)&modelOffset->cte_pct,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((dF)&ANN_PBA_PBA_UDF::ann_pba_pba_cte_pct)),
	Descriptor(37, Attribute::SCALAR_DOUBLE,	"disc_rate_mult_fixed", -1, (size_t)&modelOffset->disc_rate_mult_fixed,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((dF)&ANN_PBA_PBA_UDF::ann_pba_pba_disc_rate_mult_fixed)),
	Descriptor(38, Attribute::SCALAR_INT,	"final_period", -1, (size_t)&modelOffset->final_period,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((iF)&ANN_PBA_PBA_UDF::ann_pba_pba_final_period)),
	Descriptor(39, Attribute::SCALAR_INT,	"hedging_flag", -1, (size_t)&modelOffset->hedging_flag,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((iF)&ANN_PBA_PBA_UDF::ann_pba_pba_hedging_flag)),
	Descriptor(40, Attribute::SCALAR_STRING,	"hedging_output_file_best_estimate", -1, (size_t)&modelOffset->hedging_output_file_best_estimate,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((sF)&ANN_PBA_PBA_UDF::ann_pba_pba_hedging_output_file_best_estimate)),
	Descriptor(41, Attribute::SCALAR_STRING,	"hedging_output_file_existing_hedges", -1, (size_t)&modelOffset->hedging_output_file_existing_hedges,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((sF)&ANN_PBA_PBA_UDF::ann_pba_pba_hedging_output_file_existing_hedges)),
	Descriptor(42, Attribute::SCALAR_DOUBLE,	"hedging_stat_scale_factor_max", -1, (size_t)&modelOffset->hedging_stat_scale_factor_max,
	...
	Descriptor(47, Attribute::SCALAR_DOUBLE,	"reins_adj", -1, (size_t)&modelOffset->reins_adj,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((dF)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_adj)),
	Descriptor(48, Attribute::SCALAR_INT,	"reins_flag", -1, (size_t)&modelOffset->reins_flag,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((iF)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_flag)),
	Descriptor(49, Attribute::SCALAR_DOUBLE,	"reins_pct", -1, (size_t)&modelOffset->reins_pct,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((dF)&ANN_PBA_PBA_UDF::ann_pba_pba_reins_pct)),
	Descriptor(50, Attribute::SCALAR_INT,	"start_period", -1, (size_t)&modelOffset->start_period,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((iF)&ANN_PBA_PBA_UDF::ann_pba_pba_start_period)),
	Descriptor(51, Attribute::SCALAR_INT,	"tax_capzd_prem_amortzn_yrs", -1, (size_t)&modelOffset->tax_capzd_prem_amortzn_yrs,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((iF)&ANN_PBA_PBA_UDF::ann_pba_pba_tax_capzd_prem_amortzn_yrs)),
	Descriptor(52, Attribute::SCALAR_INT,	"tax_flag", -1, (size_t)&modelOffset->tax_flag,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((iF)&ANN_PBA_PBA_UDF::ann_pba_pba_tax_flag)),
	Descriptor(53, Attribute::SCALAR_STRING,	"valn_results_scen_amt", -1, (size_t)&modelOffset->valn_results_scen_amt,
				Descriptor::NOT_SHARED, false, false, (size_t)0, 0, Feature((sF)&ANN_PBA_PBA_UDF::ann_pba_pba_valn_results_scen_amt)),
	Descriptor(54, Attribute::SCALAR_INT,	"working_res_defn", -1, (size_t)&modelOffset->working_res_defn,
	...
	&ANN_PBA_PBA::descriptor_0[46],
	&ANN_PBA_PBA::descriptor_0[47],
	&ANN_PBA_PBA::descriptor_0[48],
	&ANN_PBA_PBA::descriptor_0[49],
	&ANN_PBA_PBA::descriptor_0[50],
	&ANN_PBA_PBA::descriptor_0[51],
	&ANN_PBA_PBA::descriptor_0[52],
	&ANN_PBA_PBA::descriptor_0[53],
	&ANN_PBA_PBA::descriptor_0[54],
	nullptr};
	const size_t ANN_PBA_PBA::sDescriptorCount = 55;
...
ANN_PBA_PBA::ANN_PBA_PBA(const xstring &modelClassName,
		...
		const char *name, ModelClass* arrayPersistentObj) : ModelClass(65, ANN_PBA_PBA::descriptorTable, arrayPersistentObj), Variable(*this)
  , sm_bond_is(0)
  , sm_bond_pv(0)
  , sm_bond_ym(0)
  , sm_mtg_is(0)
...
#pragma warning(pop)
   ...
   mMOCD = descriptorTable;
	...
	for (int cf_no = 1; cf_no <= 65; cf_no++)
#ifdef CF_MEMORY
		setPtr_col(cf_no, 0);
#else
...
BitArray ANN_PBA_PBA::dataVariables(55);
