#ifndef __BOND_AIG_H_
#define __BOND_AIG_H_
...
class BOND_AIG_persistent_object;
class BOND_AIG : public ModelClass {
...
public :
	...
	size_t sizeofThis() const { return sizeof(BOND_AIG); }
...
double prop_bond_portfolio;
 ...
 // Column Definition Begins
 	...
 	ColumnAccessor < mCFStaticData_0 > asset_maturity_period_aig;
 	ColumnAccessor < mCFStaticData_0 > asset_yld_aig;
...
#endif	MICROSOFT
	#define DESCRIPTOR_TABLE BOND_AIG::descriptor_0
#ifdef MICROSOFT
#pragma warning(pop)
#endif	MICROSOFT
	Attribute::Proxy <xstring, DESCRIPTOR_TABLE > bond_id_aig;
  inline xstring	Get_bond_id_aig() {
		return bond_id_aig; }
  inline void Set_bond_id_aig(const xstring &v) {
		bond_id_aig.setValue(v); }
...
void after_startup(int decrement = 0);
 void copy_names();
 void passDataVariables(BOND_AIG* target) { passDataVars(target); }
...
//factory
static BOND_AIG* makeThis(int isSubmodel, ModelClass *owner, BOND_AIG* peer, 
					int mainRebase, const xstring &name, BOND_AIG_persistent_object* arrayTemplate);

//constructor
BOND_AIG(const xstring &modelClassName, int isSubmodel, ModelClass *owner, ModelClass *peer, 
...
BOND_AIG(int columnCount, Descriptor* mocd[], Product* persObj);
...
class BOND_AIG_persistent_object : public BOND_AIG {
...
public :
  ...
  bool isPersistentObject() const { return true; }
   ...
   sm_calling_model->subTotalKey(keys);
	...
	void findTargetColumns() {BOND_AIG::findTargetColumns();}
	void createAllShare() {
		BOND_AIG::createAllShare();
}
...
//factory
static BOND_AIG_persistent_object* makeThis(int isSubmodel, ModelClass *owner, BOND_AIG* peer, 
					int mainRebase, const xstring &name, BOND_AIG_persistent_object* arrayTemplate, bool fixedArray);

//constructor
BOND_AIG_persistent_object(const xstring &modelClassName,
